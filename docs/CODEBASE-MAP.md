...
| `src/domain/` | **All computation lives here.** `statements` `ratios` `scoring` `industry` `graph` `valuechain` `concepts` (concept vocabulary) `presenters` (shape → API contract) `news` (feed presentation: derived source, resolved tags) `taxonomy` (industry tree: paths, descendants, roll-up) `drivers` (causal claims + the joint backtest that checks them) `fiscal` (fiscal→calendar quarter alignment). Governed by `docs/INVESTMENT-METHODOLOGY.md` — the two must always agree |
