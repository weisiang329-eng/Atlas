...
import { KIND_LABEL, KIND_TONE, type Relation } from "@/lib/mock/graph";
  ...
  if (relations.length === 0) {
    return (
      <div className="rounded-panel border border-dashed border-border px-6 py-10 text-center">
        <p className="text-sm text-muted">该公司暂无已录入关系。</p>
      </div>
    );
  }
      ...
      {Array.from(groups.entries()).map(([kind, rows]) => (
        <div key={kind}>
          <div className="mb-2 flex items-center gap-2">
            <Badge tone={KIND_TONE[kind as Relation["kind"]]}>{KIND_LABEL[kind as Relation["kind"]]}</Badge>
            <span className="text-2xs text-faint">{rows.length} 条</span>
          </div>
            ...
            {rows.map((r, i) => {
              ...
              return (
                <li key={i} className="flex items-center gap-3 py-2">
                  ...
                  <span className="text-2xs text-faint">置信 {r.confidence}</span>
                </li>
              );
            })}
          </ul>
